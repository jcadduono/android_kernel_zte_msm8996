...
#define NVT_FW_VERSION "nvt_fw_version_2nd"
#define NVT_BASELINE "nvt_baseline_2nd"
#define NVT_RAW "nvt_raw_2nd"
#define NVT_DIFF "nvt_diff_2nd"
...
return:
	n.a.
*******************************************************/
void nvt_change_mode_2nd(uint8_t mode)
{
	uint8_t buf[8] = {0};
...
info_retry:
	...
	buf[0] = 0x78;
	CTP_I2C_READ_2ND(ts_2nd->client, I2C_FW_Address, buf, 17);
	fw_ver = buf[1];
	x_num = buf[3];
	y_num = buf[4];
	button_num = buf[11];
...
static void nvt_read_mdata(uint32_t xdata_addr, uint32_t xdata_btn_addr)
{
	...
	for (i = 0; i < ((dummy_len + data_len) / XDATA_SECTOR_SIZE); i++) {
		...
		for (j = 0; j < (XDATA_SECTOR_SIZE / I2C_TANSFER_LENGTH); j++) {
			...
			CTP_I2C_READ_2ND(ts_2nd->client, I2C_FW_Address, buf, I2C_TANSFER_LENGTH + 1);
	...
	CTP_I2C_READ_2ND(ts_2nd->client, I2C_FW_Address, buf, (TOUCH_KEY_NUM * 2 + 1));
...
static int32_t nvt_baseline_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_clear_fw_status_2nd() != 0)
		return -EAGAIN;

	nvt_change_mode_2nd(TEST_MODE_1);

	if (nvt_check_fw_status_2nd() != 0)
		return -EAGAIN;
	...
	nvt_change_mode_2nd(NORMAL_MODE);
...
static int32_t nvt_diff_open(struct inode *inode, struct file *file)
{
	...
	nvt_change_mode_2nd(TEST_MODE_2);
...
*******************************************************/
int32_t nvt_extra_proc_init_2nd(void)
{
