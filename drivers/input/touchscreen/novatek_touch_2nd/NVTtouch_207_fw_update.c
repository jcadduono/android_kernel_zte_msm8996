...
extern struct nvt_ts_data *ts_2nd;
extern int32_t CTP_I2C_READ_2ND(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len);
extern int32_t CTP_I2C_READ_DUMMY_2ND(struct i2c_client *client, uint16_t address);
extern int32_t CTP_I2C_WRITE_2ND(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len);
extern void nvt_hw_reset_2nd(void);
extern void nvt_bootloader_reset_2nd(void);
extern int32_t nvt_check_fw_reset_state_2nd(RST_COMPLETE_STATE check_reset_state);
extern void nvt_sw_reset_idle_2nd(void);
...
static int32_t update_firmware_request(char *filename)
{
	...
	ret = request_firmware(&fw_entry, filename, &ts_2nd->client->dev);
	if (ret) {
...
static int32_t Check_FW_Ver(void)
{
	...
	buf[1] = 0x01;
	buf[2] = 0x47;
	ret = CTP_I2C_WRITE_2ND(ts_2nd->client, I2C_FW_Address, buf, 3);
	if (ret < 0) {
	...
	buf[1] = 0x00;
	buf[2] = 0x00;
	ret = CTP_I2C_READ_2ND(ts_2nd->client, I2C_FW_Address, buf, 3);
	if (ret < 0) {
...
static int32_t Check_CheckSum(void)
{
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		if (fw_bin_size > (i * SIZE_64KB)) {
			...
			WR_Filechksum[i] = i + 0x00 + 0x00 + (((len_in_blk - 1) >> 8) & 0xFF)
						+ ((len_in_blk - 1) & 0xFF);
			...
			ret = CTP_I2C_WRITE_2ND(ts_2nd->client, I2C_HW_Address, buf, 7);
			if (ret < 0) {
			...
			while (1) {
				...
				buf[0] = 0x00;
				buf[1] = 0x00;
				ret = CTP_I2C_READ_2ND(ts_2nd->client, I2C_HW_Address, buf, 2);
				if (ret < 0) {
				...
				retry++;
				if (unlikely(retry > 5)) {
					NVT_ERR("Check 0xAA failed, buf[1]=0x%02X, retry=%d\n", buf[1], retry);
					return -EPERM;
				}
			}
			...
			if (WR_Filechksum[i] != RD_Filechksum[i]) {
				NVT_LOG("RD_Filechksum[%d]=0x%04X, WR_Filechksum[%d]=0x%04X\n",
						i, RD_Filechksum[i], i, WR_Filechksum[i]);
				NVT_LOG("firmware checksum not match!!\n");
				return 0;
			}
		}
	}
...
*******************************************************/
static int32_t Init_BootLoader(void)
{
	uint8_t buf[64] = {0};
	int32_t ret = 0;

	nvt_sw_reset_idle_2nd();

	buf[0] = 0x00;
	buf[1] = 0x00;
	ret = CTP_I2C_WRITE_2ND(ts_2nd->client, I2C_HW_Address, buf, 2);
	if (ret < 0) {
...
static int32_t Erase_Flash(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		ret = CTP_I2C_WRITE_2ND(ts_2nd->client, I2C_HW_Address, buf, 5);
		if (ret < 0) {
		...
		while (1) {
			...
			buf[1] = 0x00;
			buf[2] = 0x00;
			ret = CTP_I2C_READ_2ND(ts_2nd->client, I2C_HW_Address, buf, 3);
			if (ret < 0) {
...
static int32_t Write_Flash(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		for (j = 0; j < min(fw_entry->size - i * 256, (size_t)256); j += 32) {
			...
			ret = CTP_I2C_WRITE_2ND(ts_2nd->client, I2C_FW_Address, buf, 33);
			if (ret < 0) {
		...
		if (fw_entry->size - Flash_Address >= 256)
			...
			tmpvalue = (Flash_Address >> 16) + ((Flash_Address >> 8) & 0xFF)
			+ (Flash_Address & 0xFF) + 0x00 + (fw_entry->size - Flash_Address - 1);
		...
		buf[7] = tmpvalue;
		ret = CTP_I2C_WRITE_2ND(ts_2nd->client, I2C_HW_Address, buf, 8);
		if (ret < 0) {
...
return:
	n.a.
*******************************************************/
void Boot_Update_Firmware_2nd(struct work_struct *work)
{
	int32_t ret = 0;
