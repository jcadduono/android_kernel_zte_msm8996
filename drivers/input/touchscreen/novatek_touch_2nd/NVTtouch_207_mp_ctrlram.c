...
extern int32_t nvt_clear_fw_status_2nd(void);
extern void nvt_change_mode_2nd(uint8_t mode);
extern int32_t nvt_check_fw_status_2nd(void);
...
static int32_t nvt_read_short_rxrx(void)
{
	...
	if (nvt_clear_fw_status_2nd()) {
		NVT_ERR("clear fw status failed!\n");
		return -EAGAIN;
	}

	nvt_change_mode_2nd(TEST_MODE_SHORT_RXRX);

	if (nvt_check_fw_status_2nd()) {
	...
	buf[0] = 0x00;
	CTP_I2C_READ_2ND(ts_2nd->client, I2C_FW_Address, buf, IC_RX_CFG_SIZE * 2 + 1);
	...
	if (nvt_check_fw_reset_state_2nd(RESET_STATE_NORMAL_RUN)) {
	...
	nvt_change_mode_2nd(TEST_MODE_SHORT_RXRX1);
...
static int32_t nvt_read_open(void)
{
	...
	nvt_change_mode_2nd(TEST_MODE_OPEN);
	...
	for (i = 0; i < AIN_TX_NUM; i++) {
		...
		CTP_I2C_READ_2ND(ts_2nd->client, I2C_FW_Address, buf, AIN_RX_NUM * 2 + 1);
...
static int32_t nvt_read_btn(void)
{
	...
	nvt_change_mode_2nd(TEST_MODE_BUTTON);
	...
	CTP_I2C_WRITE_2ND(ts_2nd->client, I2C_FW_Address, buf, 3);
	buf[0] = 0x00;
	CTP_I2C_READ_2ND(ts_2nd->client, I2C_FW_Address, buf, Btn_Number * 2 + 1);
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if (mutex_lock_interruptible(&ts_2nd->lock)) {
		return -ERESTARTSYS;
	}
...
return:
	Executive outcomes. 0---succeed. -1---failed.
*******************************************************/
int32_t nvt_mp_proc_init_2nd(void)
{
	NVT_proc_selftest_entry = proc_create("nvt_selftest_2nd", 0444, NULL, &nvt_selftest_fops);
