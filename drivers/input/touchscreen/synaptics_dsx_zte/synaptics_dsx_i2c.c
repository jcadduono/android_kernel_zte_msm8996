...
static struct workqueue_struct *syna_rmi4_init_wq;
static struct work_struct	syna_rmi4_init_work;
...
static void synaptics_rmi4_init_callback(struct work_struct *work)
{
	if (i2c_add_driver_async(&synaptics_rmi4_i2c_driver) != 0)
		pr_err("%s: fail to register driver\n", __func__);
}

int synaptics_rmi4_bus_init(void)
{
	pr_notice("%s: touch screen init\n", __func__);

	syna_rmi4_init_wq = create_singlethread_workqueue("syna_rmi4_init_wq");
	INIT_WORK(&syna_rmi4_init_work, synaptics_rmi4_init_callback);

	queue_work(syna_rmi4_init_wq, &syna_rmi4_init_work);
