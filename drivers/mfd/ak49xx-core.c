...
/*	#endif */
	...
	if (ak49xx->smartpa_rst1_gpio)
	{
		...
		pr_err("%s():SPA_RST1(%d) value(%d)",
			__func__, ak49xx->smartpa_rst1_gpio,
	...
	if (ak49xx->smartpa_rst2_gpio)
	{
		...
		pr_err("%s():SPA_RST2(%d) value(%d)",
			__func__, ak49xx->smartpa_rst2_gpio,
...
static int __init ak49xx_init(void)
{
	...
	ret1 = slim_driver_register(&ak4961_slim_driver);
	if (ret1 != 0) {
	...
	ret2 = spi_register_driver(&ak4961_spi_driver);
	if (ret2 != 0) {
