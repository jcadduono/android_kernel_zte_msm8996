...
MODULE_PARM_DESC(no_start, "do not start the work queue; for debugging via usern");
...
static int tfa98xx_input_open(struct input_dev *dev)
{
	...
	dev_dbg(tfa98xx->codec->dev, "opening device filen");
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		dev_dbg(&tfa98xx->i2c->dev,
			"DSP not loaded, cannot start tap-detectionn");
		return -EIO;
	}
...
static void tfa98xx_input_close(struct input_dev *dev)
{
	...
	dev_dbg(tfa98xx->codec->dev, "closing device filen");
...
static int tfa98xx_register_inputdev(struct tfa98xx *tfa98xx)
{
	...
	if (!input) {
		dev_err(tfa98xx->codec->dev, "Unable to allocate input devicen");
		return -ENOMEM;
	}
	...
	if (err) {
		dev_err(tfa98xx->codec->dev, "Unable to register input devicen");
		goto err_free_dev;
	}

	dev_dbg(tfa98xx->codec->dev, "Input device for tap-detection registered: %sn",
		input->name);
	tfa98xx->input = input;
	return 0;
...
static void __tfa98xx_inputdev_check_register(struct tfa98xx *tfa98xx, bool unregister)
{
	...
	for (i = 0; i < tfaContMaxProfile(tfa98xx->handle); i++) {
		if (strnstr(tfaContProfileName(tfa98xx->handle, i), ".tap",
			strlen(tfaContProfileName(tfa98xx->handle, i)))) {
			tap_profile = true;
	...
	if (tfa98xx->input)
		dev_info(tfa98xx->codec->dev, "Input device already registered, skippingn");
	else
...
static int tfa98xx_dbgfs_otc_get(void *data, u64 *val)
{
	...
	if (otc->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			pr_err("Unable to check DSP access: %dn", err);
			return -EIO;
		} else if (err == Tfa98xx_Error_NoClock) {
			if (otc->rd_valid) {
				/* read cached value */
				*val = otc->rd_value;
				pr_err("Returning cached value of OTC: %llun", *val);
			} else {
				pr_info("OTC value never read!n");
				return -EIO;
			}
			return 0;
		}
...
static int tfa98xx_dbgfs_otc_set(void *data, u64 val)
{
	...
	if (val != 0 && val != 1) {
		pr_err("Unexpected value %llunn", val);
		return -EINVAL;
	}
	mutex_lock(&tfa98xx->dsp_lock);
	status = tfa98xx_open(tfa98xx->handle);
	...
	if (otc->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			otc->wr_value = val;
			otc->triggered = true;
			pr_err("Deferring write to OTC (%d)n", otc->wr_value);
			return 0;
		}
	}
...
static int tfa98xx_dbgfs_mtpex_set(void *data, u64 val)
{
	...
	if (val != 0) {
		pr_err("Can only clear MTPEX (0 value expected)n");
		return -EINVAL;
	}
	...
	if (mtpex->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			pr_err("Deferring write to MTPEX (%d)n", mtpex->wr_value);
			return 0;
		}
	}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (ret) {
		pr_info("Calibration start failed (%d), deferring...n", ret);
		calib->triggered = true;
	} else {
		pr_info("Calibration startedn");
	}
...
#define TFA98XX_DEBUGFS_REG_CREATE_FILE(__reg, __name)				\
	debugfs_create_file(TOSTRING(__reg) "-" TOSTRING(__name), S_IRUGO, dbg_reg_dir,\
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	if (tfa98xx->flags & TFA98XX_FLAG_SAAM_AVAILABLE) {
		dev_dbg(tfa98xx->dev, "Adding pga_gain debug interfacen");
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (profile == tfa98xx_profile) {
		...
		if (ready) {
			...
			if (err) {
				pr_err("Write vstep error: %dn", err);
			} else {
				pr_err("Successfully changed vstep index!n");
			}
		}
		mutex_unlock(&tfa98xx->dsp_lock);
#ifndef TFA98XX_ALSA_CTRL_PROF_CHG_ON_VOL
	...
	pr_err("vstep:%d, (control value: %d) - profile %dn", new_vstep,
								 value, profile);
	return (err == 0);
}
		       ...
		       struct snd_ctl_elem_info *uinfo)
{
	...
	pr_err("vsteps count: %d [prof=%d]n", tfacont_get_max_vstep(tfa98xx->handle, profile),
			profile);
	return 0;
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (prof_idx < 0) {
		pr_err("tfa98xx: sample rate [%d] not supported for this mixer profile [%d].n",
			   tfa98xx->rate, new_profile);
		return 0;
	}
	pr_err("selected container profile [%d]n", prof_idx);
	...
	if (ready) {
		...
		if (err) {
			pr_info("Write profile error: %dn", err);
		} else {
			pr_err("Changed to profile %d (vstep = %d)n", prof_idx,
							tfa98xx_vsteps[0]);
		}
	}
...
static void get_profile_basename(char *buf, char *profile)
{
	...
	pch = strnchr(profile, ((int) strlen(profile)), '.');
	idx = pch - profile;
...
static int get_profile_from_list(char *buf, int id)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (bprof->item_id == id) {
			strlcpy(buf, bprof->basename, sizeof(bprof->basename));
			return 0;
		}
	}
...
static int is_profile_in_list(char *profile, int len)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (!strncmp(bprof->basename, profile, len))
			return 1;
	}
...
static int add_sr_to_profile(struct tfa98xx *tfa98xx, char *basename, int len, int profile)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (!strncmp(bprof->basename, basename, len)) {
			/* add supported samplerate for this profile */
			sr = tfa98xx_get_profile_sr(tfa98xx->handle, profile);
			if (!sr) {
				pr_err("unable to identify supported sample rate for %sn", bprof->basename);
				return -EPERM;
			}
			...
			if (idx < 0 || idx >= TFA98XX_NUM_RATES) {
				pr_err("invalid index for samplerate %dn", idx);
				return -EPERM;
			}
			...
			pr_err("added profile:samplerate = [%d:%d] for mixer profile: %sn",
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_err("%ldn", ucontrol->value.integer.value[0]);
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	pr_err("profile name :%sn", name);
	pr_err("%s : -----tfa98xx----2--n", __func__);
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		if (is_profile_in_list(bprofile->basename, bprofile->len) == 0 &&
			...
			pr_err("profile added [%d]: %sn", bprofile->item_id, bprofile->basename);
...
{
#if (KERNEL_VERSION(3, 18, 24) >= LINUX_VERSION_CODE)
	struct snd_soc_dapm_context *dapm = &tfa98xx->codec->dapm;
#else
	struct snd_soc_dapm_context *dapm = &tfa98xx->codec->dapm;/* snd_soc_codec_get_dapm(tfa98xx->codec); */
#endif
	struct snd_soc_dapm_widget *widgets;
...
static int tfa98xx_register_dsp(struct tfa98xx *tfa98xx)
{
	...
	for (i = 0; i < tfa98xx_cnt_max_device(); i++) {
		if (tfaContGetSlave(i, &slave) != Tfa98xx_Error_Ok) {
			pr_err("------------%s: i=%d - dev = 0x%xn", __func__, i, slave);
			goto reg_err;
			}
		pr_err("%s: i=%d - dev = 0x%xn", __func__, i, slave);
	...
	if (handle != -1) {
		...
		dev_info(&tfa98xx->i2c->dev,
				"Registered DSP instance with handle %dn",
								handle);
...
reg_err:
	dev_err(&tfa98xx->i2c->dev,
		"Unable to match I2C address 0x%x with a container devicen",
							tfa98xx->i2c->addr);
	return -EINVAL;
}
...
static void tfa98xx_unregister_dsp(struct tfa98xx *tfa98xx)
{
	...
	dev_info(&tfa98xx->i2c->dev, "Un-registered DSP instance with handle %dn",
							tfa98xx->handle);
}
					...
					unsigned short value)
{
	...
	if (tfa98xx_devices[handle]) {
		...
		if (!tfa98xx || !tfa98xx->regmap) {
			pr_err("No tfa98xx regmap availablen");
			return Tfa98xx_Error_Bad_Parameter;
		}
retry:
		ret = regmap_write(tfa98xx->regmap, subaddress, value);
		if (ret < 0) {
			pr_warn("i2c error, retries left: %dn", retries);
			if (retries) {
				retries--;
		...
		if (tfa98xx_kmsg_regs)
			dev_dbg(&tfa98xx->i2c->dev, "  WR reg=0x%02x, val=0x%04x %sn",
								subaddress, value,
								ret < 0 ? "Error!!" : "");

		if (tfa98xx_ftrace_regs)
			tfa98xx_trace_printk("tWR     reg=0x%02x, val=0x%04x %sn",
								subaddress, value,
								ret < 0 ? "Error!!" : "");
	} else {
		pr_err("No device availablen");
...
retry:
		...
		if (ret < 0) {
			pr_warn("i2c error at subaddress 0x%x, retries left: %dn", subaddress, retries);
			if (retries) {
				retries--;
				msleep(I2C_RETRY_DELAY);
				goto retry;
		...
		*val = value & 0xffff;
		...
		if (tfa98xx_kmsg_regs)
			dev_dbg(&tfa98xx->i2c->dev, "RD   reg=0x%02x, val=0x%04x %sn",
								subaddress, *val,
								ret < 0 ? "Error!!" : "");
		if (tfa98xx_ftrace_regs)
			tfa98xx_trace_printk("tRD     reg=0x%02x, val=0x%04x %sn",
								subaddress, *val,
								ret < 0 ? "Error!!" : "");
	} else {
		pr_err("No device availablen");
				...
				int len, unsigned char value[])
{
	...
	if (tfa98xx_devices[handle] && tfa98xx_devices[handle]->i2c) {
		...
		if (err != ARRAY_SIZE(msgs)) {
			dev_err(&tfa98xx_client->dev, "read transfer error %dn",
									err);
			error = Tfa98xx_Error_Fail;
		}

		if (tfa98xx_kmsg_regs)
			dev_dbg(&tfa98xx_client->dev, "RD-DAT reg=0x%02x, len=%dn",
								reg, len);
		if (tfa98xx_ftrace_regs)
			tfa98xx_trace_printk("ttRD-DAT reg=0x%02x, len=%dn",
					reg, len);
	} else {
		pr_err("No device availablen");
		error = Tfa98xx_Error_Fail;
...
retry:
		...
		if (ret == len) {
			if (tfa98xx_kmsg_regs)
				dev_dbg(&tfa98xx->i2c->dev, "  WR-RAW len=%dn", len);
			if (tfa98xx_ftrace_regs)
				tfa98xx_trace_printk("ttWR-RAW len=%dn", len);
			return Tfa98xx_Error_Ok;
		}
		pr_err("  WR-RAW (len=%d) Error I2C send size mismatch %dn", len, ret);
		error = Tfa98xx_Error_Fail;
	} else {
		pr_err("No device availablen");
		error = Tfa98xx_Error_Fail;
							...
							unsigned int mask)
{
	...
	if (reg & errs & ~mask) {
		...
		dev_info(tfa98xx->codec->dev, "status_reg events: 0x%xn", reg);
	}
...
static void tfa98xx_tapdet_check_update(struct tfa98xx *tfa98xx)
{
	...
	if (!gpio_is_valid(tfa98xx->irq_gpio)) {
		...
		dev_dbg(tfa98xx->codec->dev,
			"Polling for tap-detection: %s (%d; 0x%x, %d)n",
		...
		dev_dbg(tfa98xx->codec->dev,
			"SPK interrupt for tap-detection: %s (%d; 0x%x, %d)n",
				enable ? "enabled":"disabled",
				tfa98xx->tapdet_open, tfa98xx->tapdet_profiles,
				tfa98xx_profile);
	...
	if (!tfa98xx->tapdet_enabled && enable) {
		...
		err = tfa98xx_dsp_write_mem_word(tfa98xx->handle, XMEM_TAP_ACK, 0,
							Tfa98xx_DMEM_XMEM);
		if (err)
			pr_info("Unable to write to XMEMn");
		...
		if (count > 1)
			pr_info("Had to run %d times to ack SPKS at initn", count);
	}
...
static void tfa98xx_interrupt_setup_tfa1(struct tfa98xx *tfa98xx)
{
	...
	dev_dbg(&tfa98xx->i2c->dev, "Initial interrupts setup: ICR = 0x%04xn", ie_reg);
}
...
static void tfa98xx_interrupt_restore_tfa1(struct tfa98xx *tfa98xx)
{
	...
	if (ie_reg != handles_local[tfa98xx->handle].interrupt_enable[0]) {
		...
		dev_dbg(&tfa98xx->i2c->dev, "Restored interrupts: ICR = 0x%04xn",
									ie_reg);
	} else {
		dev_dbg(&tfa98xx->i2c->dev, "No interrupt restore neededn");
	}
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	pr_err("%s : -----tfa98xx----1--n", __func__);
	if (!cont) {
		pr_err("Failed to read %sn", fw_name);
		return;
	}

	pr_err("loaded %s - size: %zun", fw_name,
					cont ? cont->size : 0);

	container = kzalloc(cont->size, GFP_KERNEL);
	if (!container) {
		release_firmware(cont);
		pr_err("Error allocating memoryn");
		return;
	}
	...
	pr_err("%.2s%.2sn", container->version, container->subversion);
	pr_err("%.8sn", container->customer);
	pr_err("%.8sn", container->application);
	pr_err("%.8sn", container->type);
	pr_err("%d ndevn", container->ndev);
	pr_err("%d nprofn", container->nprof);
	pr_err("%d indexn", container->index[0].type);

	tfa_err = tfa_load_cnt(container, container_size);
	if (tfa_err != tfa_error_ok) {
		dev_err(tfa98xx->dev, "Cannot load container file, abortingn");
		return;
	}
	...
	if (tfa98xx->handle < 0) {
		dev_err(tfa98xx->dev, "Cannot register with DSP, abortingn");
		return;
	}

	if (tfa_probe(tfa98xx->i2c->addr << 1, &handle) != Tfa98xx_Error_Ok) {
		dev_err(tfa98xx->dev, "Failed to probe TFA98xx @ 0x%.2xn", tfa98xx->i2c->addr);
		return;
	}
	...
	if (strcmp(dflt_prof_name, "")) {
		...
		for (i = 0; i < tfaContMaxProfile(tfa98xx->handle); i++) {
			if (strcmp(tfaContProfileName(tfa98xx->handle, i),
				...
				pr_err("%s : -----tfa98xx----2--n", __func__);
				dev_err(tfa98xx->dev,
					"changing default profile to %s (%d)n",
		...
		if (i >= tfaContMaxProfile(tfa98xx->handle))
			dev_info(tfa98xx->dev,
				"Default profile override failed (%s profile not found)n",
				dflt_prof_name);
	}


	tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_OK;
	pr_err("Firmware init completen");

	if (no_start != 0)
		return;
	pr_err("%s : -----tfa98xx----3--n", __func__);
...
static void tfa98xx_tapdet(struct tfa98xx *tfa98xx)
{
	...
	switch (mem) {
	case 0xffffffff:
		pr_info("More than 4 taps detected! (flagTapPattern = -1)n");
		btn = BTN_0;
		break;
	case 0xfffffffe:
		pr_info("Single tap detected! (flagTapPattern = -2)n");
		btn = BTN_0;
		break;
	case 0:
		pr_info("Unrecognized pattern! (flagTapPattern = 0)n");
		btn = BTN_0;
		break;
	default:
		pr_info("Detected pattern: %dn", mem);
		btn = BTN_0 + mem;
		break;
	}
	...
	if (count > 1)
		pr_info("Had to run %d times to ack SPKSn", count);
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (!tfa98xx->input) {
		 ...
		 */
		val = snd_soc_read(tfa98xx->codec, TFA98XX_STATUSREG);
		pr_err("SYS_STATUS0: 0x%04xn", val);
		...
		if (!(val & TFA98XX_STATUSREG_CLKS) ||
			...
			pr_err("Misc errors detected: STATUS_FLAG0 = 0x%xn", val);
		...
		(val & TFA98XX_STATUS_FLAGS1_TDMLUTER))
			pr_err("TDM related errors: STATUS_FLAG1 = 0x%xn", val);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		pr_err("Skipping tfa_start (no FW: %d)n", tfa98xx->dsp_fw_state);
		return;
	}

	if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
		pr_err("Stream already started, skipping DSP power-onn");
		return;
	}
...
static void tfa98xx_interrupt(struct work_struct *work)
{
	...
	if (out2) {
		...
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTMWSRC, out2) > 0) {
			...
			if (manwait1 > 0) {
				pr_info("entering wait for source staten");
				tfa98xx->count_wait_for_source_state++;

				/* set AMPC and AMPE to make sure the amp is enabled */
				pr_info("setting AMPC and AMPE to 1 (default)n");
				...
				pr_info("leaving wait for source staten");
						...
						struct snd_soc_dai *dai)
{
	...
	basename = kzalloc(MAX_CONTROL_NAME, GFP_KERNEL); /* add by mxp patch for fixing memory leak */
	if (!basename)
		return -ENOMEM;
	 ...
	 */
	for (prof = 0; prof < nprof; prof++) {
		if (!strncmp(basename, tfaContProfileName(tfa98xx->handle, prof), len)) {
			/* Check which sample rate is supported with current profile,
			 * and enforce this.
			 */
			sr = tfa98xx_get_profile_sr(tfa98xx->handle, prof);
			if (!sr)
				dev_info(codec->dev, "Unable to identify supported sample raten");
	...
	tfa98xx->rate_constraint.count += 1;
	kfree(basename); /* dll modify by nxp patch 20160830 */
...
static int tfa98xx_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	pr_err("fmt=0x%xn", fmt);

	/* Supported mode: regular I2S, slave, or PDM */
	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
		if ((fmt & SND_SOC_DAIFMT_MASTER_MASK) != SND_SOC_DAIFMT_CBS_CFS) {
			dev_err(codec->dev, "Invalid Codec master moden");
			return -EINVAL;
		}
		break;
	case SND_SOC_DAIFMT_PDM:
		break;
	default:
		dev_err(codec->dev, "Unsupported DAI format %dn",
			     ...
			     struct snd_soc_dai *dai)
{
	...
	pr_err("Requested rate: %d, sample size: %d, physical size: %dn",
			rate, snd_pcm_format_width(params_format(params)),
			snd_pcm_format_physical_width(params_format(params)));

	if (params_channels(params) > 2) {
		pr_warn("Unusual number of channels: %dn", params_channels(params));
	}
	...
	if (prof_idx < 0) {
		pr_err("tfa98xx: invalid sample rate %d.n", rate);
		return -EINVAL;
	}
	pr_err("mixer profile:container profile = [%d:%d]n", tfa98xx_mixer_profile, prof_idx);
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	dev_dbg(&tfa98xx->i2c->dev, "state: %dn", mute);

	if (!(tfa98xx->flags & TFA98XX_FLAG_DSP_START_ON_MUTE))
		return 0;

	if (no_start) {
		pr_err("no_start parameter set no tfa_start or tfa_stop, returningn");
		return 0;
	}
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	if (!tfa98xx->tfa98xx_wq)
		return -ENOMEM;
	pr_err("%s : -----tfa98xx----1-1-n", __func__);
	...
	pr_err("Container loading requested: %dn", ret);

#if (KERNEL_VERSION(3, 16, 0) > LINUX_VERSION_CODE)
	codec->control_data = tfa98xx->regmap;
	ret = snd_soc_codec_set_cache_io(codec, 8, 16, SND_SOC_REGMAP);
	if (ret != 0) {
		dev_err(codec->dev, "Failed to set cache I/O: %dn", ret);
		return ret;
	}
#endif
...
static void tfa98xx_irq_tfa2(struct tfa98xx *tfa98xx)
{
	...
	pr_info("interrupt1: 0x%.4x (enabled: 0x%.4x)n", out1, en1);
	pr_info("interrupt2: 0x%.4x (enabled: 0x%.4x)n", out2, en2);
	pr_info("interrupt3: 0x%.4x (enabled: 0x%.4x)n", out3, en3);
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
	...
	dev_info(&tfa98xx->i2c->dev, "interrupt: 0x%04x (enabled: 0x%04x)n", val, ie);
#ifdef DEBUG
	if (!(val & ie)) {
		unsigned int ireg;
		...
		dev_dbg(&tfa98xx->i2c->dev, "ICR: 0x%04xn", ireg);
	}
#endif
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	pr_err("%s : enter ++++++++++n", __func__);
	tfa98xx->reset_gpio = of_get_named_gpio(np, "reset-gpio", 0);
	pr_err("%s : enter +++++1+++++  reset_gpio=%dn", __func__, tfa98xx->reset_gpio);
	if (tfa98xx->reset_gpio < 0)
		dev_err(dev, "No reset GPIO provided, will not HW reset devicen");

	tfa98xx->irq_gpio =  of_get_named_gpio(np, "irq-gpio", 0);
	if (tfa98xx->irq_gpio < 0)
		dev_err(dev, "No IRQ GPIO provided.n");
			     ...
			     const struct i2c_device_id *id)
{
	...
	pr_err("%s----1------n", __func__);

	if (!i2c_check_functionality(i2c->adapter, I2C_FUNC_I2C)) {
		dev_err(&i2c->dev, "check_functionality failedn");
		return -EIO;
	}
	pr_err("%s+++++1+++++n", __func__);
	...
	if (IS_ERR(tfa98xx->regmap)) {
		...
		dev_err(&i2c->dev, "Failed to allocate register map: %dn",
			ret);
		goto err;
	}
	pr_err("%s+++++3+++++n", __func__);
	i2c_set_clientdata(i2c, tfa98xx);
	mutex_init(&tfa98xx->dsp_lock);
	init_waitqueue_head(&tfa98xx->wq);
	pr_err("%s+++++3+1++++n", __func__);
	if (np) {
		ret = tfa98xx_parse_dt(&i2c->dev, tfa98xx, np);
		if (ret) {
			dev_err(&i2c->dev, "Failed to parse DT noden");
			pr_err("%s+++++3+1+0+++n", __func__);
			goto err;
		}
		if (no_start)
			tfa98xx->irq_gpio = -1;
	} else {
		pr_err("%s+++++3+1+1+++n", __func__);
		tfa98xx->reset_gpio = -1;
		tfa98xx->irq_gpio = -1;
	}
	pr_err("%s+++++3+2++++n", __func__);
	...
	pr_err("%s+++++4+0++++n", __func__);
	if (gpio_is_valid(tfa98xx->irq_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, tfa98xx->irq_gpio,
			GPIOF_DIR_IN, "TFA98XX_INT");
		pr_err("%s+++++4+2+1+0++ret=%dn", __func__, ret);
		if (ret)
			goto err;
	}
	pr_err("%s+++++5+++++n", __func__);
	/* Power up! */
	...
	if (no_start == 0) {
		...
		case 0x92: /* tfa9891 */
			...
			break;
		case 0x97:
			pr_info("TFA9897 detectedn");
			tfa98xx->flags |= TFA98XX_FLAG_DSP_START_ON_MUTE;
	    tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
			tfa98xx->flags |= TFA98XX_FLAG_TFA9897_FAM_DEV;
			break;
		default:
			pr_info("Unsupported device revision (0x%x)n", reg & 0xff);
			return -EINVAL;
		}
	}
	pr_err("%s+++++6+++++n", __func__);
	...
	if (ret < 0) {
		dev_err(&i2c->dev, "Failed to register TFA98xx: %dn", ret);
		goto err_off;
	}
	pr_err("%s+++++7+++++n", __func__);
	if (gpio_is_valid(tfa98xx->irq_gpio) &&
		!(tfa98xx->flags & TFA98XX_FLAG_SKIP_INTERRUPTS)) {
		...
		ret = devm_request_threaded_irq(&i2c->dev,
					...
					"tfa98xx", tfa98xx);
		if (ret != 0) {
			dev_err(&i2c->dev, "Failed to request IRQ %d: %dn",
					gpio_to_irq(tfa98xx->irq_gpio), ret);
			goto err_off;
		}
		tfa98xx_interrupt_setup(tfa98xx);
	} else {
		dev_info(&i2c->dev, "Skipping IRQ registrationn");
		/* disable feature support if gpio was invalid */
		tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
	}
	pr_err("%s+++++8+++++n", __func__);
#ifdef CONFIG_DEBUG_FS
	...
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs filesn");
	ret = device_create_bin_file(&i2c->dev, &dev_attr_reg);
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs filesn");

	pr_info("%s Probe completed successfully!n", __func__);
...
static int __init tfa98xx_i2c_init(void)
{
	...
	pr_info("TFA98XX driver version %sn", TFA98XX_VERSION);
