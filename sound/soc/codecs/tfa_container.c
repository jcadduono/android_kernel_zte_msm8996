...
nxpTfaDeviceList_t *tfaContGetDevList(nxpTfaContainer_t *cont, int dev_idx)
{
	...
	if (cont->index[dev_idx].type != dscDevice) {
		pr_err("%s --------2-----cont->index[dev_idx].type=%d\n", __func__, cont->index[dev_idx].type);
		/* return NULL; */
	}
	/* pr_err("%s --------3-----cont->index[dev_idx].type=%d\n",__func__,cont->index[dev_idx].type); */
...
static void cont_get_devs(nxpTfaContainer_t *cont)
{
	...
	pr_err("-----1----%s g_devs=%d cont->ndev=%d\n", __func__, g_devs, cont->ndev);
	g_devs = cont->ndev;
	...
	for (i = 0; i < g_devs; i++) {
		...
		pr_err("-----2----%s g_devs=%d cont->ndev=%d\n", __func__, g_devs, cont->ndev);
		g_devs = cont->ndev;
...
static enum Tfa98xx_Error tfaContWriteVstepMax2(int dev_idx,
	nxpTfaVolumeStepMax2File_t *vp, int vstep_idx, int vstep_msg_idx)
{
	...
	for (i = 0; i <= vstep_idx; i++) {
		...
		for (j = 0; j < nrMessages; j++) {
		    ...
		    (regInfo->NrOfRegisters * sizeof(uint32_t)+sizeof(regInfo->NrOfRegisters)+size));
			/* message length */
			msgLength = ((msgInfo->MessageLength.b[0] << 16) +
						(msgInfo->MessageLength.b[1] << 8) +
			...
			if (i == vstep_idx) {
				...
				if ((vstep_msg_idx >= TFA_MAX_VSTEP_MSG_MARKER) || (vstep_msg_idx == j)) {
					 ...
					 * The algoparams and mbdrc msg id will be changed to the reset type
					 when SBSL=0
					 * if SBSL=1 the msg will remain unchanged. It's up to the tuning engineer
					    to choose
					    the 'without_reset'
					 * types inside the vstep. In other words: the reset msg is applied during
					    SBSL==0
					...
					if (msgInfo->MessageType != 3) {
						if (modified == 1) {
							...
							err = tfa_dsp_msg_id(dev_idx, (msgLength-1) * 3,
							      (const char *)msgInfo->ParameterData,
							       cmdid_changed);
							if (err != Tfa98xx_Error_Ok)
								return err;
						} else {
							/* Send cmdId + payload to dsp */
							err = tfa_dsp_msg(dev_idx, msgLength * 3,
							(const char *)msgInfo->CmdId);
							if (err != Tfa98xx_Error_Ok)
								return err;
						}
			...
			if (msgInfo->MessageType == 3) {
				...
				size += sizeof(msgInfo->MessageType) + sizeof(msgInfo->MessageLength) +
				msgLength;
			} else {
				/* MessageLength is in words (3 bytes) */
				size += sizeof(msgInfo->MessageType) +
						sizeof(msgInfo->MessageLength) +
						sizeof(msgInfo->CmdId) + ((msgLength-1) * 3);
			}
		}
		size += sizeof(regInfo->NrOfRegisters) +
 ...
 */
enum Tfa98xx_Error tfaContWriteFile(int dev_idx,  nxpTfaFileDsc_t *file, int vstep_idx,
										int vstep_msg_idx)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	...
	case volstepHdr:
		if (tfa98xx_dev_family(dev_idx) == 2) {
			err = tfaContWriteVstepMax2(dev_idx, (nxpTfaVolumeStepMax2File_t *)hdr,
	...
	case presetHdr:
		...
		err = tfa98xx_dsp_write_preset(dev_idx, size,
	...
	case configHdr:
		...
		err = tfa98xx_dsp_write_config(dev_idx, size,
	...
	case drcHdr:
		if (hdr->version[0] == NXPTFA_DR3_VERSION) {
			...
			err = tfa98xx_dsp_write_drc(dev_idx, size,
...
enum Tfa98xx_Error tfaRunWriteDspMem(Tfa98xx_handle_t dev, nxpTfaDspMem_t *cfmem)
{
	...
	for (i = 0; i < cfmem->size; i++) {
		if (tfa98xx_cnt_verbose)
			...
			(cfmem->address+i), cfmem->words[i]);

		error = tfa98xx_dsp_write_mem_word(dev, (cfmem->address+i),
...
enum Tfa98xx_Error get_sample_rate_info(int dev_idx, nxpTfaProfileList_t *prof,
	nxpTfaProfileList_t *previous_prof, int fs_previous_profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaBitfield_t *bitf;
	unsigned int i;
...
int tfaContGetCalProfile(int dev_idx)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		if (strnstr(tfaContProfileName(dev_idx, prof), ".cal",
			strlen(tfaContProfileName(dev_idx, prof))) != NULL) {
			cal_idx = prof;
...
int tfaContIsTapProfile(int dev_idx, int prof_idx)
{
	...
	if (strnstr(tfaContProfileName(dev_idx, prof_idx), ".tap",
		strlen(tfaContProfileName(dev_idx, prof_idx))) != NULL) {
...
{
	pr_err("-------1-----%s:dev_idx=%d  g_devs=%d\n", __func__, dev_idx, g_devs);
...
{
	nxpTfaLiveDataList_t *nextlivedata = (nxpTfaLiveDataList_t *)((char *)livedata +
					(livedata->length*4) + sizeof(nxpTfaLiveDataList_t) - 4);
