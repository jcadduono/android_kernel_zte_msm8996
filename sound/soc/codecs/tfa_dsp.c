...
const char *tfa98xx_get_i2c_status_id_string(int status)
{
	...
	case Tfa98xx_I2C_Req_Invalid_Seq:
		p_id_str = "in case the DSP expects some commands in a specific order";
		break;
	case Tfa98xx_I2C_Req_Invalid_Param:
		p_id_str = "Generic error";
		break;
	case Tfa98xx_I2C_Req_Buffer_Overflow:
		p_id_str = "too many parameters, memory integrity is not guaranteed";
		break;
	default:
...
enum Tfa98xx_Error tfaRunSpeakerBoost(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	if ((value == 1) && (!istap_prof)) {
		...
		/*modify by dll for no boot sound 20161128 start*/
		switch (TFA_GET_BF(handle, REV) & 0xff) {
		case 0x97:
		err = tfa98xx_dsp_reset(handle, 1);
		err = tfa98xx_dsp_reset(handle, 0);
		pr_err("tfa98xx------Reset 97 at first cold start up");
		break;
		default:
		break;
		}
		/*modify by dll for no boot sound 20161128  end*/
	} else if (istap_prof)  {
		/* Save the current profile and set the vstep to 0 */
...
enum Tfa98xx_Error tfaRunSpeakerCalibration(Tfa98xx_handle_t handle, int profile)
{
	...
	if ((strnstr(tfaContProfileName(handle, profile), ".cal", strlen(tfaContProfileName(handle, profile))) == NULL)
		&& (tfa98xx_dev_family(handle) == 2)) {
...
enum Tfa98xx_Error tfaRunUnmute(Tfa98xx_handle_t handle)
{
	...
	if (tfa98xx_dev_family(handle) == 2) {
		...
		if ((TFA_GET_BF(handle, CFE) != 0) && (manstate == 9)) {
			TFA_SET_BF_VOLATILE(handle, AMPE, 1);/* handset mode */
			return err;
		}
		...
		/* SL: Enable everything,there will be sound after clock and sound applied */
		TFA_SET_BF_VOLATILE(handle, SBSL, 1);
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	for (dev = 0; dev < devcount; dev++) {
		 ...
		 */
		if (strnstr(tfaContProfileName(dev, next_profile), ".standby",
			strlen(tfaContProfileName(dev, next_profile))) != NULL) {
...
enum tfa_error tfa_stop(void)
{
	...
	for (dev = 0; dev < devcount; dev++) {
		...
		pr_debug("Stopping device [%s]\n", tfaContDeviceName(dev));
			if (tfa98xx_dev_family(dev) == 2) {
			do {
				if (strnstr(tfaContProfileName(dev, stop_profile), ".standby",
					sizeof(tfaContProfileName(dev, stop_profile))) != NULL)
	...
	if (error == Tfa98xx_Error_Ok) {
		...
		if (TFA_GET_BF(handle, MTPOTC) && ((handles_local[handle].rev & 0xff) == 0x88)) {
			...
			for (i = 0; i < spkr_count; i++) {
				handles_local[handle].mohm[i] = tfa_read_reg(handle,
			...
			error = tfa_dsp_cmd_id_write_read(handle, MODULE_SPEAKERBOOST,
				SB_PARAM_GET_RE0, nr_bytes, bytes);
			if (error == Tfa98xx_Error_Ok) {
